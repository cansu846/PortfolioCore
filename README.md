<h1 align="center">🧾 Personal Portfolio Web Application</h1>

<p align="center">
  <em>A modern, responsive CV website built with ASP.NET Core 6.0 and a full tech stack.</em>
</p>

---

## 🚀 Technologies Used

<ul>
  <li><b>ASP.NET Core 6.0 (Web API & MVC)</b> – Application framework and backend logic</li>
  <li><b>MSSQL</b> – Relational database</li>
  <li><b>Entity Framework Core & LINQ</b> – ORM and data access layer</li>
  <li><b>HTML, CSS, Bootstrap</b> – Frontend layout and styling</li>
  <li><b>JavaScript</b> – Dynamic interactions and enhanced UX</li>
</ul>

---

## 🎯 Project Purpose

This project is designed to <b>showcase my software development skills</b> and present my resume in a web-based, interactive format. Rather than a static PDF, this dynamic CV application delivers a richer and more engaging professional profile.

---

## 🛠️ Features

- ✅ Sections for personal info, education, experience, and skills  
- ✅ Responsive design (mobile-friendly)  
- ✅ CRUD operations  
- ✅ (Optional) Admin panel for easy updates  

---
## 🔎 Portfolio Filtering Feature

<div style="border: 1px solid #ddd; border-left: 6px solid #007bff; padding: 16px; background-color: #f9f9f9; font-family: Arial, sans-serif;">
  <h3 style="margin-top: 0; color: #007bff;">📂 Filter Portfolios by Category</h3>
  <p style="color: #333;">
    This web application allows users to filter portfolio items by category using an intuitive UI. When a category (e.g., <em>App Programming</em>, <em>AI</em>) is selected, the corresponding items are fetched and displayed dynamically via <strong>AJAX</strong>, without a page refresh.
  </p>
  <p style="color: #333;">
    This feature enhances the user experience by making it easy to browse different sections of the portfolio based on user preferences, without reloading the entire page.
  </p>
</div>

--- 

## ⚙️ Setup Instructions

1. **Clone the repository**
   ```bash
   git clone https://github.com/your-username/cv-project.git

---

## 📸 Screenshots

![Screenshot 2025-05-01 230245](https://github.com/user-attachments/assets/a66f9b21-3845-4c97-ad7c-c19f8b764c3d)
![Screenshot 2025-05-01 230319](https://github.com/user-attachments/assets/3ec3a01e-3f2e-4fe2-bb0c-b7aae1a9c820)
![Screenshot 2025-05-01 230344](https://github.com/user-attachments/assets/95214917-959b-4dca-87fa-a4fd4f900c63)
![Screenshot 2025-05-01 230400](https://github.com/user-attachments/assets/04d5fca3-d9ed-4fc1-8b2a-33e3cb8e3b61)
![Screenshot 2025-05-01 230420](https://github.com/user-attachments/assets/ca2813cf-7dec-4b9e-888c-9360fac8fd77)
![Screenshot 2025-05-03 114323](https://github.com/user-attachments/assets/b3486b3e-007e-4049-a19d-658a8cb92204)
![Screenshot 2025-05-03 113519](https://github.com/user-attachments/assets/47ef84f8-3fbb-4f3a-ac53-396ca491420f)
![Screenshot 2025-05-01 230525](https://github.com/user-attachments/assets/60cb84af-2d4e-4dda-ac3a-8f84b83d41e6)
![Screenshot 2025-05-01 230624](https://github.com/user-attachments/assets/5e2ffbc0-b3a3-440b-b2b1-78a474e457c5)
![Screenshot 2025-05-01 232258](https://github.com/user-attachments/assets/12018108-74c1-40d2-afcb-40db2fc7091f)
![Screenshot 2025-05-01 232307](https://github.com/user-attachments/assets/f3a171cc-6cca-4024-aa17-7cf1dcadba59)
![Screenshot 2025-05-01 232414](https://github.com/user-attachments/assets/158e69e5-81d5-4a6f-84bc-71e6e2274796)
![Screenshot 2025-05-01 232423](https://github.com/user-attachments/assets/01db8419-f49d-4011-a581-a55f5d5c79b8)
![Screenshot 2025-05-01 230652](https://github.com/user-attachments/assets/e6075b9b-b965-42fc-bb5e-e67f489a6a5c)
